...
getSubNodes m (l, r)
    | l == r = getSubNodes (m-next) (l+next, r) 
...
layout tr  = layout' tr 1
    where layout' (Branch v l r) depth = Branch (v, (xpos v, depth)) 
                            (layout' l (depth+1)) (layout' r (depth+1))
...
layout2 tr = xshift $ helper tr 0 1
    where helper (Branch v l r) xpos depth = Branch (v, (xpos, depth))
...
xshift tr = helper tr
    where helper (Branch (v, (x, y)) l r) = Branch (v, (x+xoff, y))
            (helper l) (helper r)
          helper Empty = Empty
          xoff = 1-(minx tr)
...
compactTree tr@(Branch (v, (x, y)) l r) = 
    Branch (v, (x, y)) (shiftTr l' offset) (shiftTr r' (-offset))
        where minD = min (maxDepth l) (maxDepth r) 
              offset = (distance - 2) `div` 2
              getx = fst . snd
              distance = foldl min (maxDistance tr)
...
split' xs = (take m xs, drop (m+1) xs)
    where nl = countChar '(' xs
          nr = countChar ')' xs
          m = length $ takeWhile (== False) 
                [(nl!!i == nr!!i) && xs!!i == ',' | i <- [0..length xs]]

countChar :: Char -> [Char] -> [Int]
countChar x s = helper x s 0
    where helper x (y:ys) n = if x==y then ((n+1):(helper x ys (n+1)))
...
preInTree preord inord = helper prein
    where prein = [(p, head (DL.elemIndices p inord)) | p <- preord]
          n = length preord
          helper [] = Empty
          helper ((x1, x2):zs) = Branch x1 
